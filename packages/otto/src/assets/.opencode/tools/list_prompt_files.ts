...
export default tool({
  description: "List managed prompt files and whether each file is user-editable.",
  args: {},
  async execute(_args): Promise<string> {
    const { baseUrl, token } = resolveInternalApiConfiguration()
    const response = await fetch(`${baseUrl}/internal/tools/prompts/files/list`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`List prompt files request failed (${response.status}): ${body}`)
    }
