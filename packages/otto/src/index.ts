...
import { createComponentLogger, createLogger, logger } from "./logging/logger.js"
...
const main = async (): Promise<void> => {
  ...
  const parentLogger = command.name === "doctor" ? createLogger({ logLevel: "error" }) : logger
  const commandLogger = createComponentLogger(command.name, parentLogger)
