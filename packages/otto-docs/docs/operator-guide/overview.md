---
id: overview
title: Operator Guide Overview
description: How to run, verify, and recover Otto safely.
---

The operator guide is task-oriented. Use these runbooks when operating installed
Otto services and diagnosing incidents.

## Runbooks

- [Setup and First Start](./setup-and-first-start.md)
- [Daily Self-Learning (EOD)](./daily-self-learning-eod.md)
- [Notification Policy and Watchdog Alerts](./notification-policy-and-watchdog-alerts.md)
- [Service Lifecycle and Health Checks](./service-lifecycle-and-health-checks.md)
- [Update Workflow](./update-workflow.md)
- [Docs Platform Rollout and Rollback Runbook](./docs-platform-rollout-and-rollback.md)
- [Static Docs vs Live Docs Surface](./docs-surface-static-vs-live.md)
- [Incident Triage](./incident-triage.md)
- [Troubleshooting Runtime State and Processes](./troubleshooting-runtime-state-and-processes.md)
- [Troubleshooting Limits and Pagination](./troubleshooting-limits-and-pagination.md)
- [Troubleshooting Auth and Live Access](./troubleshooting-auth-and-live-access.md)

All procedures should include verification commands and expected signals.
