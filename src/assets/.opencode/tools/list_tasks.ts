...
export default tool({
  description: "List scheduled Otto tasks.",
  args: {},
  async execute(): Promise<string> {
    const { baseUrl, token } = resolveInternalApiConfiguration()
    const response = await fetch(`${baseUrl}/internal/tools/tasks/list`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`List tasks request failed (${response.status}): ${body}`)
    }
